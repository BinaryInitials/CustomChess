...
public class EngineMinMaxNoMateDectionExperiment implements Engine {
	...
	public EngineMinMaxNoMateDectionExperiment(int team){
		this.team = team;
...
//					move.setScore(9000000);
		...
		for(Move move : moves) {
			...
			if((isCheck(newBoard) && !isPieceThreatenedNew(newBoard, move.getPieceAfterMove()) || (isMateTwoConsecutiveMoves(newBoard)))) {
				...
				for(Move oppMove : oppMoves) {
					...
					int thisCheckScore = keepMovingUntilMate(newNewBoard, team);
				...
				for(Move oppMove : oppMoves) {
					...
					if(isCheck(newNewBoard, team) && !isPieceThreatenedNew(newNewBoard, oppMove.getPieceAfterMove())) {
						...
						for(Move myMove : myMoves) {
							...
							int thisCheckScore = -keepMovingUntilMate(newNewNewBoard, -team, 3, -1000000000, 100000000, 1, 3);
...
//		}
	...
	private boolean isMateTwoConsecutiveMoves(Board board) {
	...
	public int alphaBetaMinMaxInitializer(Board board, List<Move> rootMoves){
		nodeMap.clear();
		moveMap.clear();
		return alphaBetaMinMax(board, team, MAX_DEPTH, -PSEUDO_INFINITE, PSEUDO_INFINITE, 1, null, rootMoves);
	}
...
//			moves = getAllMoveForTeam(board, maximizing*team);
		...
		if(maximizing>0){
			...
			for(Move move : moves){
				...
				int bestScore;
				bestScore = alphaBetaMinMax(newBoard, team, depth-1, alpha, beta, -1, parentMove, null);
...
//			moves = filterUnecessaryMoves(board, moves);
			for(Move move : moves){
				Board newBoard = move.updateBoard(board);
				nodeMap.put(parentMove, nodeMap.get(parentMove)+1);
				int bestScore = alphaBetaMinMax(newBoard, team, depth-1, alpha, beta, 1, parentMove, null);
	...
	public int keepMovingUntilMate(Board board, int team){
		return keepMovingUntilMate(board, team, 7 , -1000000000, 100000000, 1, 7);
		/*
	...
	public int keepMovingUntilMate(Board board, int team, int depth, int alpha, int beta, int maximizing, int MAX_DEPTH){
		if(isCheckMate(board, -team)){
		...
		if(maximizing>0){
			...
			for(Move move : moves){
				Board newBoard = move.updateBoard(board);
				if(isCheck(newBoard, -team) || isMateTwoConsecutiveMoves(newBoard)){
					/*
					 ...
					 * 
					 * Ok this case was the motivation for the second condition. 
					 * rnbq1rk1/pppp1ppp/8/8/2B5/2B5/P4PPP/R2Q1RK1 w - - 0 1
					 * The engine should know to move the queen next to the bishop when the opponent has a vulnerable castle. 
					 * Unfortunately this can't be discovered by a simple minmax with maxdepth<9 or by perpetual checks. If this method works, we can remove the isMyNextMoveLeadToCheck nonsense.
					 */
					int bestScore = keepMovingUntilMate(newBoard, team, depth-1, alpha, beta, -1, MAX_DEPTH);
