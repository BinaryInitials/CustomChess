...
public class EngineMinMaxNoMateDectionMAXDEPTH7 implements Engine {
	...
	public EngineMinMaxNoMateDectionMAXDEPTH7(int team){
		this.team = team;
...
//		MAX_NODE = 127;
	...
	public Move findMove(Board board, int iteration, boolean isConsole, boolean isDeveloperMode, List<Move> moveHistory) {
		...
		for(Move move: moves) {
			Board newBoard = MoveUtil.updateBoardSimple(board, move);
		...
		for(Move move : moves) {
			...
			if(isCheck(newBoard, -team) && !isPieceThreatenedNew(newBoard, move.getPieceAfterMove())) {
				...
				for(Move oppMove : oppMoves) {
					Board newNewBoard = MoveUtil.updateBoardSimple(newBoard, oppMove);
				...
				for(Move oppMove : oppMoves) {
					...
					if(isCheck(newNewBoard, team) && !isPieceThreatenedNew(newNewBoard, oppMove.getPieceAfterMove())) {
						...
						for(Move myMove : myMoves) {
							Board newNewNewBoard = MoveUtil.updateBoardSimple(newNewBoard, myMove);
